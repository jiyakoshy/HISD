...
class EditCampusContactPage extends React.Component {
    ...
    deleteCampusContact(){
        ...
        promise.then((response) => this.displayMessage('CIC successfully deleted.', 'SAVED'))
        .catch(reason => {
            this.props.actions.ajaxCallError();
            this.setState({ subtext: 'There was a problem deleting this CIC. Operation canceled.', dialogAction: '' });
            this.showDialog();
        });
    }
    ...
    delete(){
        this.setState({ subtext: 'Do you want to delete this CIC?', dialogAction: 'DELETE' });
        this.showDialog();
    }

    
    onSavingSucceeds(response){          
        this.displayMessage('CIC successfully updated.', 'SAVED');
    }
    ...
    isValidForm(){
        ...
        if(this.state.employeeName.length == 0){
            ...
            isValid = false;
            this.displayMessage('Error: Contact name not changed. No need to save CIC record.', '');
        }
        return isValid;
    }
            ...
            <IfAnyGranted expected={['Admin', 'Principal']} actual={this.props.userProps.user.role} unauthorized={<ErrorPage />}>
                <div id="page-wrapper">
                    <div className="container-fluid">
                        <PageHeader title="Edit CIC"  isDropDownDisabled={true} campus={this.props.campus} />
                        <div className="row">
