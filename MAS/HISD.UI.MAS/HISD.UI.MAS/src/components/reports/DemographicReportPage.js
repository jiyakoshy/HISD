...
class DemographicReportPage extends React.Component {
    ...
    runReport() {
        if (this.isValidForm() == true) {
            let params = { StartDate: Utilities.getODataDate(this.state.reportStartDate), EndDate: Utilities.getODataDate23hours(this.state.reportEndDate), SchoolID: this.state.schoolID };
            this.props.actions.loadDemographicReport(params)
                .then(reportData => this.loadReportJSON());
        }
    }
    loadReportJSON() {
        this.props.actions.loadAllCampusContacts()
            .then(campusContacts => {
                if (this.props.reportData.length > 0) {
                        ...
                        this.props.campusContacts.map(campC => {
                            if (ele.EducationOrgNaturalKey == campC.EducationOrgNaturalKey) {
                                if (ele.CICName == '') {
                                    ...
                                    ele.CICEmployeeID = campC.StaffNaturalKey;
                                    ...
                                    ele.CICEmployeeID = ele.CICEmployeeID + "," +campC.StaffNaturalKey;
                                    if (campC.ElectronicMailAddress != null)
                    ...
                    this.setState({ perRelationshipLogCompletion: reportDataArray });
                }
                else {
    ...
    principalApprovalFormat(cell, row){
        if(row.PrincipalApproval == ''){
            return row.PrincipalApproval = 'Pending';
        }else{ return row.PrincipalApproval;}
    }
    
    formatMenteeEndDate(cell, row) {
        return Utilities.getFormattedDate(row.MenteeLatestHireDate);         
    }
            ...
            <IfAnyGranted expected={['Admin']} actual={this.props.userProps.user.role} unauthorized={<ErrorPage />}>
                <div id="page-wrapper">
                    <div className="container-fluid">
                        <PageHeader title="Mentor/Mentee Demographic Report" campus={this.props.campusName} hideCampusDrpDwn="Hide"/>
                            ...
                            <div className="col-md-12 col-lg-12 col-sm-12">
                                <div className="white-box">
                                    ...
                                    <BootstrapTable data={this.state.perRelationshipLogCompletion} hover condensed pagination options={options} search exportCSV>
                                        ...
                                        <TableHeaderColumn dataField="LocalOrganizationCode" dataSort dataAlign="left" width={"100px"}>PS Dept ID</TableHeaderColumn>
                                        <TableHeaderColumn dataField="CICName" datasort dataAlign="left" width={"100px"} tdStyle={{whiteSpace: 'normal', wordWrap: 'break-word'}}>CIC Name</TableHeaderColumn>
                                        ...
                                        <TableHeaderColumn dataField="RelationshipStartDate" dataSort dataAlign="left" width={"100px"} dataFormat={this.dateFormatter} thStyle={{whiteSpace: 'normal', wordWrap: 'break-word'}}>Start Date</TableHeaderColumn>
                                        <TableHeaderColumn dataField="RelationshipEndDate" dataSort dataAlign="left" width={"100px"} dataFormat={this.dateFormatter} thStyle={{whiteSpace: 'normal', wordWrap: 'break-word'}}>End Date</TableHeaderColumn>
                                        <TableHeaderColumn dataField="PrincipalApproval" dataFormat={this.principalApprovalFormat} dataSort dataAlign="center" width={"100px"} tdStyle={{whiteSpace: 'normal', wordWrap: 'break-word'}} thStyle={{whiteSpace: 'normal', wordWrap: 'break-word', maxHeight: '20px'}}>* Principal Approval</TableHeaderColumn>
                                        ...
                                        <TableHeaderColumn dataField="MenteeLatestHireDate" dataSort dataAlign="center" width={"100px"} dataFormat={this.formatMenteeEndDate} tdStyle={{whiteSpace: 'normal', wordWrap: 'break-word'}} thStyle={{whiteSpace: 'normal', wordWrap: 'break-word'}}>Mentee End Date</TableHeaderColumn>
...
function mapDispatchToProps(dispatch) {
    return { actions: bindActionCreators(Object.assign({}, campusActions, reportsActions,adminActions, ajaxActions), dispatch) };
}
