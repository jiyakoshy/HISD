...
namespace Mshp.Service
{
    public class CalendarController : ODataController
    {
        ...
        {
            IQueryable<Calendar> response = db.CalendarSet.Where(p => p.CalendarId == key);
        ...
        public IHttpActionResult Put([FromODataUri] int key, Calendar update)
        {
            ...
            if (update == null || key != update.CalendarId)
                return BadRequest();

            var original = db.CalendarSet.Where(p => p.CalendarId == key).FirstOrDefault();
            if (original == null)
                return NotFound();
