...
export function loadActivityLogsByMenteeID(menteeEmployeeID, timeConfigurationID) {
    return function (dispatch) {
        ...
        return ActivityLogsApi.getActivityLogsByMenteeID(menteeEmployeeID, timeConfigurationID).then(activityLogsObj => {              
            ...
            dispatch(loadActivityLogsByMenteeIDSuccess(activityLogs));
        }).catch(error => {
            throw (error);
        });
    };
...
export function loadActivityLogsByMenteeIDSuccess(activityLogs) {
    return { type: types.LOAD_ACTIVITY_LOGS_BY_MENTEEID_SUCCESS, activityLogs };
}
