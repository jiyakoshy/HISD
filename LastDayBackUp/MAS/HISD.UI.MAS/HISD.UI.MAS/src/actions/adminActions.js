...
export function loadAllActivityCodesDropDown() {
    return function (dispatch) {
        dispatch(beginAjaxCall());
        return AdminApi.getAllActivityCodesDropDown().then(activityCodes => {
            dispatch(loadAllActivityCodesSuccess(activityCodes.value));
        }).catch(error => {
            throw (error);
        });
...
//activityObj
export function loadActivityTypeSuccess(data) {
    return { type: types.LOAD_ALL_ACTIVITY_TYPE, data };
}

export function loadActivityType(activityID) {
    return function (dispatch) {
        return AdminApi.getActivityTypeAPI(activityID).then(item => {
            let obj = {};
            obj.ActivityStandardItemID = item.ActivityStandardItemID;
            obj.ActivityStandardItemName = item.ActivityStandardItemName;
            obj.EffectiveStartDate = new Date(Utilities.getDateTime(item.EffectiveStartDate));
            obj.EffectiveEndDate = new Date(Utilities.getDateTime(item.EffectiveEndDate));
            dispatch(loadActivityTypeSuccess(obj));
        }).catch(error => {
            throw (error);
        });
    };
...
export function updateActivityType(payload, activityId) {
    return function (dispatch) {
        dispatch(beginAjaxCall());
        return AdminApi.updateActivityTypeAPI(payload, activityId).then(response => {
            dispatch(updateActivityTypeSuceess(payload));
        }).catch(error => {
            throw (error);
        });
    };
...
export function updateActivityTypeSuceess(activityStandards) {
    return { type: types.UPDATE_ACTIVITY_TYPE_SUCCESS, activityStandards };
}
...
export function createActivityStandard(activityStandard) {
    return function (dispatch) {
        dispatch(beginAjaxCall());
        return AdminApi.createActivityStandard(activityStandard).then(response => {
            dispatch(createActivityStandardSuccess(response));
        }).catch(error => {
            throw (error);
        });
    };
...
export function createActivityStandardSuccess(activityStandards) {
    return { type: types.CREATE_ACTIVITY_STANDARD_SUCCESS, activityStandards };
}
...
export function updateMentorAgreementPDF(siteContent, PDFContent, specialElementHandlers) {
    return function (dispatch) {
        dispatch(beginAjaxCall());
        return SharePointApi.updateMentorAgreementPDF(siteContent, PDFContent, specialElementHandlers)
            .then(response => {
                dispatch(updateMentorAgreementPDFSuccess(siteContent));
            }).catch(error => {
                throw (error);
            });
    };
...
export function updateMentorAgreementPDFSuccess(siteContent) {
    return { type: types.UPDATE_MENTOR_AGREEMENT_PDF_SUCCESS, siteContent };
}
...
//Admin Dashboard: CampusCounts having CampusContacts
export function loadAllCampusesWithCampusContactSuccess(campusesWithCampusContact) {
    return { type: types.LOAD_ALLCAMPUSESWITHCAMPUSCONTACT_SUCCESS, campusesWithCampusContact };
}

export function loadAllCampusesWithCampusContact(timeConfigurationID) {
    return function (dispatch) {
        dispatch(beginAjaxCall());
        console.log('hi111--');
        return AdminApi.getAllCampusesWithCampusContact(timeConfigurationID).then(campusesWithCampusContact => {
            if (campusesWithCampusContact.value.length > 0) {
                dispatch(loadAllCampusesWithCampusContactSuccess(campusesWithCampusContact.value));
            }
...
// ------------------ Admin Dashboard Purpose -----------------------Ending
