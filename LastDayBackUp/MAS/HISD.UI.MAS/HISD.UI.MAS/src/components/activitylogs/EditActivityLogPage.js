...
class EditActivityLogPage extends React.Component {
    constructor(props, context) {
        ...
        this.setSelectedActivityStandards = this.setSelectedActivityStandards.bind(this);
        this.setMentoringTools = this.setMentoringTools.bind(this);
    }
                                ...
                                this.props.actions.loadAllActivityStandards()
                                    .then(activityStandards => {
                                        if (this.props.activityStandards && this.props.activityStandards.length >= 4) {
                                            this.setState({
                                                instructionalOptions: cbmStandardsFormattedForCheckbox(this.props.activityStandards[2].ActivityStandardItems),
                                                planningOptions: cbmStandardsFormattedForCheckbox(this.props.activityStandards[1].ActivityStandardItems),
                                            ...
                                            this.props.actions.loadActivityLogByID(activityLogID)
                                                .then(activityLog => {
                                                    ...
                                                    this.props.actions.loadMenteesInActiveRelationshipsByMentorID(mentorEmployeeID, timeConfigurationID, this.props.userProps.user.campusID)
                                                        .then(mentees => {
    ...
    setSelectedActivityStandards() {
        const getSelectedActivityOptions = Utilities.getSelectedActivities(this.props.activityLog.ActivityStandardItems, 'ActivityStandardItemID');
        this.setState({ selectedActivityStandardsOptions: getSelectedActivityOptions });
    }

    setMentoringTools() {
        const getSelectedMentoringToolsOptions = Utilities.getSelectedActivities(this.props.activityLog.ActivityToolItems, 'ActivityToolItemID');
        this.setState({ selectedMentoringOptions: getSelectedMentoringToolsOptions });
    }
    ...
    loadValues() {
        ...
        this.setSelectedActivityStandards();
        this.setMentoringTools();
        ...
        if (this.isValidForm()) {
            ...
            let activityLog = {
                ActivityLog: {
                    ...
                    CreateDate: this.props.activityLog.CreateDate,
    ...
    handleDeleteRow(row) {
        ...
        if (mentee.VerificationStatus) {
            ...
            this.props.actions.deleteNewActivityMenteeEdit(employeeID);
            this.props.actions.loadAllActivityMentees();
        }
    }
    ...
    checkboxMentoringFormat(cell, row) {
        if (cell in this.state.selectedMentoringOptions) {
            if (this.state.selectedMentoringOptions[cell] == true) {
                return (
