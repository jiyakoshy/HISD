    ...
    public class ActivityLogsController : ODataController
    {
        ...
        public IQueryable<TotalNumberofActivitiesByMentees> GetTotalActivitiesByMentees([FromODataUri] int timeconfigID, [FromODataUri]string campus, [FromODataUri]string stDate, [FromODataUri]string edDate)
        {
            ...
            var totalActivies = (from x in db.MentorMenteeRelationships.ToList()
                                 ...
                                 where x.RelationshipStatus != "Inactive"
                                 where x.MentorAgreement == "Accepted"
                                 //where x.RelationshipStatus.Equals("Active")
                                 //where x.RelationshipStatus.Equals("Pending")
                                 group xyz by new
                                 {
                                     x.MentorEmployeeID,
                                     x.MenteeEmployeeID,
        ...
        [HttpGet]
        [EnableQuery]
        [ODataRoute("GetMentorsInActiveRelationshipByCampusID(CampusID={campusID})")]
        public IQueryable<EmployeeInfo> GetMentorsInActiveRelationshipByCampusID([FromODataUri] string campusID)
        {
            // get all existing Mentor Mentee Relationships under HISD
            var allMASMentorMenteeRelationshipsInfo =
                db.MentorMenteeRelationships.Where(x => x.CampusID == campusID).AsEnumerable();
            // retrieve all Mentors in active relation
            String[] MentorEmployeeIDs = allMASMentorMenteeRelationshipsInfo.Select(me => me.MentorEmployeeID).Distinct().ToArray();
            
            var allActiveMentorsInfo =
            ...
            return allActiveMentorsInfo.AsQueryable<EmployeeInfo>();
        }

        // GET: odata/GetMenteesInActiveRelationshipByCampusID: Returns all existing Active Mentors Details
        [HttpGet]
        [EnableQuery]
        [ODataRoute("GetMenteesInActiveRelationshipByCampusID(CampusID={campusID})")]
        public IQueryable<MenteeInActiveRelationshipInfo> GetMenteesInActiveRelationshipByCampusID([FromODataUri] string campusID)
        {
            // get all existing Mentor Mentee Relationships under HISD
            var allMASMentorMenteeRelationshipsInfo =
                db.MentorMenteeRelationships.Where(x => x.CampusID == campusID).AsEnumerable().ToList();
            // retrieve all Mentees in active relation
            String[] MenteeEmployeeIDs = allMASMentorMenteeRelationshipsInfo.Select(me => me.MenteeEmployeeID).Distinct().ToArray();
            
            var allActiveMenteesInfo =
               (from ST in edwdb.Staffs
                join STM in edwdb.StaffElectronicEmails
                on ST.StaffNaturalKey equals STM.StaffNaturalKey into StaffEmail
                from se in StaffEmail.DefaultIfEmpty()
            ...
            var combinedMenteeInActiveRelationshipInfos = (from masAME in allMASMentorMenteeRelationshipsInfo.GroupBy( m => m.MenteeEmployeeID).Select( mg => mg.First())
                                                           join edwMenteeEMPLOYEES in allActiveMenteesInfo
                                                           on masAME.MenteeEmployeeID  equals edwMenteeEMPLOYEES.StaffNaturalKey
                                                           select new MenteeInActiveRelationshipInfo
                                                           {

                                                               //MENTEE INFO
                                                               StaffNaturalKey = edwMenteeEMPLOYEES.StaffNaturalKey,
                                                               FirstName = edwMenteeEMPLOYEES.FirstName,
                                                               LastSurname = edwMenteeEMPLOYEES.LastSurname,
                                                               MiddleName = edwMenteeEMPLOYEES.MiddleName,
                                                               LoginId = edwMenteeEMPLOYEES.LoginId,
                                                               ElectronicMailAddress = edwMenteeEMPLOYEES.ElectronicMailAddress,
                                                               JobCodeDescription = edwMenteeEMPLOYEES.JobCodeDescription,
                                                               LatestHireDate = edwMenteeEMPLOYEES.LatestHireDate,
                                                               // CAMPUS INFO
                                                               EducationOrgNaturalKey = edwMenteeEMPLOYEES.EducationOrgNaturalKey,
                                                               NameOfInstitution = edwMenteeEMPLOYEES.NameOfInstitution,
                                                               //RELATION STATUS INFO
                                                               TimeConfigurationID = masAME.TimeConfigurationID

                                                           }).AsEnumerable().ToList();

            return combinedMenteeInActiveRelationshipInfos.AsQueryable<MenteeInActiveRelationshipInfo>();
        ...
        [HttpGet]
        [EnableQuery]
        [ODataRoute("GetMenteesInActiveRelationshipByMentorID(EmployeeID={employeeID})")]
        public IQueryable<MenteeInActiveRelationshipInfo> GetMenteesInActiveRelationshipByMentorID([FromODataUri] string employeeID)
        {
            // get Relationship Details for this selected Emplyee
            var allMASMentorMenteeRelationshipsInfo =
                db.MentorMenteeRelationships.Where(x => x.MentorEmployeeID == employeeID).AsEnumerable();
            ...
            var combinedMenteeInActiveRelationshipInfos = (from masAME in allMASMentorMenteeRelationshipsInfo
                                                            join edwMenteeEMPLOYEES in allActiveMenteesInfo
                                                            on masAME.MenteeEmployeeID equals edwMenteeEMPLOYEES.StaffNaturalKey into EmpMenteesinMMR
                                                            ...
                                                            select new MenteeInActiveRelationshipInfo
                                                            {
                                                                ...
                                                                //RELATION STATUS INFO
                                                                MentorAgreement = masAME.MentorAgreement,
                                                                TimeConfigurationID = masAME.TimeConfigurationID
                                                            }).AsEnumerable();
