...
import { loadAllCampuses, loadCampusesByManager, loadCampusesByStaff, loadAllCSOs } from '../actions/campusActions';
import { loadGradeLevelsByCampusAndYear, loadGradeLevelsByCampusSuccess } from '../actions/gradeLevelsActions';
import { loadOrganizationGroup, loadPublicOrganizationGroup} from '../actions/annualsetupActions';
...
export function loadUser() {
    return function (dispatch) {
      ...
      return SharedApi.getUserProperties().then(userProps => {
        ...
        EnrollmentApi.getSchoolYear().then(schoolyearobj => {
            currentSchoolYear  = schoolyearobj.value;
            dispatch(loadAllSchoolYears());
            dispatch(loadCurrentSchoolYear());
            if (currentSchoolYear){
                dispatch(loadSchoolYearType());
            }
            ...
            dispatch(loadGradeLevelsByCampusAndYear(userProps.user.campusID, Utilities.getSchoolYearEnd(currentSchoolYear)));
            dispatch(loadCampusProfile(userProps.user.campusID, currentSchoolYear));
            dispatch(loadOrganizationGroup(currentSchoolYear));
            dispatch(loadPublicOrganizationGroup());
        });
    ...
    return function (dispatch) {
        ...
        return SharedApi.getPrincipal(newCampus.key).then(principalInfo => {
            ...
            EnrollmentApi.getSchoolYear().then(schoolyearobj => {
                ...
                let schoolyearend = Utilities.getSchoolYearEnd(currentSchoolYear);
                dispatch(loadAllCompareDaySeq(currentSchoolYear));
...
export function loadSchoolYearType() {
    return function (dispatch) {
        dispatch(beginAjaxCall());
        return SharedApi.getSchoolYearType().then(newschoolYear => {
            let newSY = '';
            let filteryear = newschoolYear.value.filter(c => c.CurrentSchoolYear == true);
            if (filteryear.length > 0)
            {
                newSY = filteryear['0'].Description;
                dispatch(loadSchoolYearTypeSuccess(newSY));
            }
        }).catch(error => {
            throw (error);
        });
...
export function loadSchoolYearTypeSuccess(newschoolYear) {
    return { type: types.LOAD_SCHOOLYEAR_TYPE_SUCCESS, newschoolYear };
}