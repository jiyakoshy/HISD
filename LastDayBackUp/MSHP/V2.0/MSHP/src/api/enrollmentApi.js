...
class enrollmentApi {
    ...
    static getCalendarReportDatesByYear(schoolYear) {
        //let serviceUrl = Config.REST_URL + "mshp/odata/Calendar?$filter=(indexof(PlanNotes,'aiver') le 0 and SchoolYear eq '" + schoolYear + "' and ReportDate ne null)&$select=SchoolYear,CompareDaySeq,ReportDate,Id,InstructionDay,PlanNotes,ReportSchedule&orderby=CompareDaySeq";
        let serviceUrl = Config.REST_URL + "mshp/odata/Calendar?$filter=(SchoolYear eq '" + schoolYear + "')&$select=SchoolYear,CompareDaySeq,ReportDate,Id,InstructionDay,PlanNotes,ReportSchedule&orderby=CompareDaySeq";
        return new Promise((resolve, reject) => {
            fetch(serviceUrl, {
                method: 'get',
                mode: 'cors',
    ...
    static getProfile(campusID, schoolYear) {
        return new Promise((resolve, reject) => {
            ...
            calendarIDPromise.then(
                function (calendarInfo) {
                    if (calendarInfo.value.length > 0)
