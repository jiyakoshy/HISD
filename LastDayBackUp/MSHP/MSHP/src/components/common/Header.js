...
class Header extends React.Component {
    ...
    onRoleDropdownChanged(item) {
        ...
        if(item.key == 'Admin' || item.key == 'Reports')
            this.props.actions.loadAllCampuses();
       
        else //load one (usually) campus for Campus user
   ...
   /*  test(){
        this.props.actions.loadCampusProfile(this.props.userProps.user.campusID, '2017-2018');
    }
 */
    ...
    render() {
        //console.log(this.props.campusProfile);

        if(this.props.userProps.user.campusID != "" && this.props.userProps.user.role == undefined){
            this.props.actions.logErrorSuccess("You are not authorized to access the Membership System.");
                ...
                <div className="navbar-header"> <a className="navbar-toggle hidden-lg hidden-md hidden-sm" href="javascript:void(0)" data-toggle="collapse" data-target=".navbar-collapse"><i className="ti-menu"></i></a>
                    ...
                    <ul className="nav navbar-top-links navbar-left">
                        <li className="dropdown hidden-xs">
                            <span href="#"><b className="hidden-xs" style={styleTitle}>Membership System</b> </span>
                        </li>
                    </ul>
...
function mapDispatchToProps(dispatch) {
    return { actions: bindActionCreators(Object.assign({}, adminActions, sharedActions, campusActions, enrollmentActions), dispatch) };
}
