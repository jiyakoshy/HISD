...
namespace Mshp.Service
{
    public class OrganizationGroupController : ODataController
    {
        ...
        {
            var response = db.OrganizationGroupSet.Where(p => p.Id == key);
        ...
        public IHttpActionResult Post(OrganizationGroup entity)
        {
            ...
            if (rowsAffected > 0)
                return ResponseMessage(Request.CreateResponse(HttpStatusCode.Created, entity));
            else
        ...
        public IHttpActionResult Put([FromODataUri] int key, OrganizationGroup update)
        {
            ...
            var original = db.OrganizationGroupSet.Where(p => p.Id == key).FirstOrDefault();
