...
class VotingSettingsUpdatePage extends React.Component {
    ...
    isValidForm() {
        console.log('----', Utilities.getDateTime(this.props.userProps.schoolYearBeginDate));
        let isValid = true;
        ...
        else if (Utilities.getDateTime(this.props.userProps.schoolYearBeginDate) >= this.state.cnStartDate) {
            isValid = false;
            this.displayMessage('Error. All the dates should be greater than school year begin date : '+ Utilities.getDateOnly(this.props.userProps.schoolYearBeginDate), '');
        }
        else if (this.state.cnStartDate >= this.state.cnEndDate) {
            isValid = false;
            this.displayMessage('Error: Invalid Candidate Nominee Dates. Candidate Nominee Start Date shoud not be greater than Candidate Nominee End Date.', '');
        }
        else if (this.state.cnEndDate >= this.state.vsStartDate) {
            isValid = false;
            this.displayMessage('Error. Candidate Nominee Date shoud not be greater than Voting Date.', '');
        }
        else if (this.state.vsStartDate >= this.state.vsEndDate) {
            isValid = false;
            this.displayMessage('Error: Invalid Voting Dates. Voting Start Date shoud not be greater than Voting End Date.', '');
        }
        else if (this.state.vsEndDate >= this.state.raStartDate) {
            isValid = false;
            this.displayMessage('Error. Result Available Date shoud not be greater than Voting Date.', '');
        }
        else if (this.state.raStartDate >= Utilities.getDateTime(this.props.userProps.schoolYearEndDate)) {
            isValid = false;
            this.displayMessage('Error. All the dates should be less than school year end date : ' + Utilities.getDateOnly(this.props.userProps.schoolYearEndDate), '');
        }
        ...
        return (
            <div id="page-wrapper">
                <div className="container-fluid">
                    <PageHeader title="Voting Settings" schoolYear={this.props.schoolYear} showSchoolDropDown="No" />
