...
class Utilities {
    ...
    static getDateOnlyWithMonthNameWeekName(theDate) {
        if (theDate === undefined || theDate === '') return '';
        let oldDate = Date.parse('01/02/1900');
        if (theDate === undefined || theDate == '' || theDate < oldDate) return '';
        let validDate = moment(theDate);
        const monthNames = ["January", "February", "March", "April", "May", "June",
            "July", "August", "September", "October", "November", "December"
        ];
        let weekName = moment(validDate).format('dddd');
        let yyyy = moment(validDate).format('YYYY');
        let mm = moment(validDate).format('MMMM'); // getMonth() is zero-based
        let dd = moment(validDate).format('D');
        return weekName + "," + mm + " " + dd + "," + yyyy; // padding
    }
